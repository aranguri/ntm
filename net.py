...
initial_memories = tf.Variable(tf.constant(1e-6, shape=(1, batch_size, memory_size)))
...
shapes = [tf.TensorShape([batch_size, None]), tf.TensorShape([None, batch_size, memory_size]), tf.TensorShape([2, batch_size, h_size_i]),
          tf.TensorShape([2, batch_size, h_size_m])]
last_state = tf.while_loop(cond, body, [tf.constant(0.0, shape=(batch_size, 0)), initial_memories, rnn_i.h, rnn_m.h], shape_invariants=[*shapes])
